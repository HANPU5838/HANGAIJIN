...
class BootstrapManager(
    ...
    fun needsPostSetup(): Boolean {
        val marker = File(homeDir, ".HAN/.post-setup-done")
        return isInstalled() && !marker.exists()
    }

    val postSetupScript: File
        get() = File(homeDir, ".HAN/post-setup.sh")
    ...
    fun getStatus(): SetupStatus =
        SetupStatus(
            ...
            platformInstalled = File(homeDir, ".HAN/.post-setup-done").exists(),
        )
    ...
    private fun setupDirectories() {
        ...
        tmpDir.mkdirs()
        wwwDir.mkdirs()
        File(homeDir, ".HAN/patches").mkdirs()
    }
